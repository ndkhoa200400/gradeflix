...
const AdminAccounts = ({ recently, keyword, currentPage, pageSize, onLoading }) => {
	...
	const headerFormatter = (column, colIndex, { sortElement, filterElement }) => {
		return (
			<div style={{ display: "flex", flexDirection: "row", justifyContent: "center", cursor: "pointer" }}>
				...
				&nbsp;
				<div variant="link" style={{ padding: "0px" }}>
	...
	const beforeSaveCell = async (oldValue, newValue, row, column, done) => {
		...
		try {
			...
			setUsers(newUsers);
			await postApiMethod(`admin/users/` + id, { studentId: newValue });
		} catch (e) {
			console.log(e);
			setShowToast(true);
			const newUsers = [...users];
	...
	const getAllUser = async () => {
		try {
			...
			const res = await getApiMethod("admin/users", params);
			const arr = [];
			...
			console.log(
				`Get user list: recently ${recently}, keyword: ${keyword}, currentPage: ${currentPage}, pageSize: ${pageSize}, totalPages: ${totalPages}`
			);
