...
const AccountQuickView = ({ show, handleClose, openModal, user, onStudentIdChange }) => {
	...
	const onSubmiting = async () => {
		setSpinning(true);
		try {
			await postApiMethod(`admin/users/` + user.id, { studentId });
		...
		>
			<Modal.Header closeButton>
				<Modal.Title style={{ fontWeight: "bold" }}>
					{user.fullname} (
					<span style={{ color: isLocked ? "red" : "green" }}>{isLocked ? "Bị khóa" : "Đang hoạt động"}</span>)
				</Modal.Title>
			</Modal.Header>
			<Modal.Body>
				<Row>
							...
							<Card.Body>
								<Form className="form-profile">
									...
									<Form.Group className="mb-3" controlId="formFullname">
										...
										<Form.Control type="text" placeholder="Nhập họ và tên" value={user.fullname} disabled={true} />
									</Form.Group>

									<Form.Group className="mb-3" controlId="formBirthday">
										<Form.Label>Ngày sinh</Form.Label>
										<Form.Control type="text" value={user.birthday} disabled={true} />
									</Form.Group>
									<Form.Group className="mb-3" controlId="formBirthday">
										<Form.Label>Ngày mở tài khoản</Form.Label>
										<Form.Control type="text" value={formatDate(user.createdAt)} disabled={true} />
									</Form.Group>
									{user.role !== "ADMIN" ? (
										<div>
											...
											<div className="d-flex" style={{ justifyContent: "flex-end" }}>
												...
												<Button
													...
													onClick={() => onSubmiting()}
													className="px-4 py-2"
													disabled={user.studentId === studentId}
												>
													Lưu
												</Button>
											</div>
