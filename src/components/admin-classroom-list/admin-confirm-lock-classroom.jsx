	...
	return (
		<Modal show={show} onHide={handleClose}>
			<Modal.Header closeButton>
				<Modal.Title>Xác nhận {!currentClassroom.active ? "mở" : "khóa"} lớp học</Modal.Title>
				{spinning ? <Spinning isFull={false} className="mx-2" /> : null}
			</Modal.Header>
			<Modal.Body>
				Bạn có chắc {!currentClassroom.active ? "mở" : "khóa"} lớp học {currentClassroom.name}
			</Modal.Body>
			<Modal.Footer>
				...
				<Button
					variant="primary"
					onClick={() => {
						onClick(currentClassroom.id, !currentClassroom.active);
					}}
				>
					Đồng ý
				</Button>
