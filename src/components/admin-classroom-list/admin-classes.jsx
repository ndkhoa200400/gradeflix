...
const AdminClasses = ({ recently, keyword, currentPage, pageSize, onLoading }) => {
	...
	const getAllClasses = async () => {
		try {
			...
			if (keyword) {
				filter.where = { or: [{ name: { ilike: `%${keyword}%` } }] };
			}
			const params = {
				pageSize: pageSize,
			...
			const res = await getApiMethod("admin/classrooms", params);

			setClassrooms(res.items);
			const totalPages = res.totalPages;
			console.log(
				`Get classroom list: recently ${recently}, keyword: ${keyword}, currentPage: ${currentPage}, pageSize: ${pageSize}, totalPages: ${totalPages}`
			);
			onLoading(res.total);
		} catch (e) {
			console.log(e);
	...
	const onClick = async (id, isLocked) => {
		...
		setSpinning(true);
		try {
			await postApiMethod(`admin/classrooms/` + id, { active: isLocked });
	...
	return (
		...
		>
			<div className="ms-2 me-auto " onClick={onViewClick} style={{ width: "100%", textAlign: "start" }}>
				<div className="fw-bold" style={{ color: isLocked ? "red" : "black" }}>
					{classroom.name}
				</div>
				Người tạo: {host ? host.fullname : ""} <br />
				Thành viên: {numTeacher} Giáo viên, {numStudent} Học viên
				<br />
				Ngày mở: {formatDate(classroom.createdAt)}
				<br />
				Trạng thái:{" "}
				<span style={{ fontWeight: "bold", color: isLocked ? "red" : "green" }}>
